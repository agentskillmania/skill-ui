...
export interface SplitDividerProps {
  ...
  maxWidth?: number;
  /** Which side the resizable pane sits on (default: 'right'). */
  side?: 'left' | 'right';
}
...
export function SplitDivider({
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      const sidebar = (
        side === 'left' ? divider.previousElementSibling : divider.nextElementSibling
  ...
  useEffect(() => {
    const handleMouseMove = (ev: MouseEvent) => {
      ...
      const delta = side === 'left' ? ev.clientX - ds.startX : ds.startX - ev.clientX;
